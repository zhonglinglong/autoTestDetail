...
def test_1002():
    ...
    # describe： 检测出租合同超过一年生成2条业绩单，其中一条未起算
    # data：房源为合租，品牌公寓，合同周期为2年
    # result：1.合同创建成功2.合同生成2条出单业绩，一条为未起算

    fileName = 'apartmentContract_1002'
    randomApartment = "SELECT a.apartment_code,a.apartment_id,hc.contract_num,hc.contract_id FROM apartment a INNER JOIN house_contract hc ON hc.contract_id = a.house_contract_id " \
                      "AND hc.is_active = 'Y' AND hc.deleted = 0 AND hc.audit_status='APPROVED' AND hc.contract_status = 'EFFECTIVE' INNER JOIN fitment_house fh on fh.house_id=hc.house_id " \
                      "AND fh.fitment_status='HANDOVER' inner join query_apartment qa on qa.apartment_code=a.apartment_code WHERE a.deleted = 0 AND a.rent_price > 0 AND a.city_code = 330100 " \
                      "AND hc.entrust_type = 'SHARE' AND a.rent_status='WAITING_RENT' AND hc.apartment_type='BRAND' AND hc.real_due_date>date_add(date(sysdate()), interval 2 YEAR) ORDER BY RAND() LIMIT 1"
    ...
    with Base() as base:
        ...
        randomHouseContract = sqlbase.serach(
            "SELECT entrust_start_date,entrust_end_date,date(sysdate()),date_add(date(sysdate()), interval 1 DAY),date_add(date(sysdate()), interval 2 year) "
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(contractAdd,1)).is_true(), 1002,
                        u'%s:出租合同 %s 新增失败，执行SQL:%s' % (fileName, contractNum, contractAdd))
        achievementSql = "select * from apartment_contract_achievement where contract_num='%s' and audit_status='AUDIT' and deleted=0 " % contractNum
        base.diffAssert(lambda test: asserts(sqlbase.waitData(achievementSql, 2)).is_true(), 1002,
                                u'%s:合同 %s 对应业绩生成异常' % (fileName, contractNum))
        achievementSqlA ="select is_active from apartment_contract_achievement where contract_num='%s' and audit_status='AUDIT' and deleted=0 and accounting_num=1" % contractNum
        achievementStatusA = sqlbase.serach(achievementSqlA)[0]
        base.diffAssert(lambda test: asserts(achievementStatusA).is_equal_to('N'), 1002,
                        u'%s:出租合同 %s 第一条业绩生成异常，生效状态期望值 N 实际值 %s' % (fileName, contractNum, achievementStatusA))
        achievementSqlB = "select is_active from apartment_contract_achievement where contract_num='%s' and audit_status='AUDIT' and deleted=0 and accounting_num=2" % contractNum
        achievementStatusB = sqlbase.serach(achievementSqlB)[0]
        base.diffAssert(lambda test: asserts(achievementStatusB).is_equal_to('UNCALCULATE'), 1002,
                        u'%s:出租合同 %s 第二条业绩生成异常，生效状态期望值 UNCALCULATE 实际值 %s' % (fileName, contractNum, achievementStatusB))
