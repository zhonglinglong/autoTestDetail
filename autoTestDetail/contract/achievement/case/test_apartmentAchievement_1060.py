...
def test_1060():
    ...
    # describe：出租合同退租终止结算生成违约业绩，记录同步到预估业绩排行榜
    # data：1、出租合同状态为有效；2、出租合同审核状态为已复审；3、应收违约金大于0；4、应收转租费大于0
    # result：1、合同状态变为退租；2、生成一条违约业绩，状态为未生效；3、违约业绩金额等于应收违约金；4、违约业绩分成记录插入到预估业绩排行榜；

    fileName = 'apartmentAchievement_1060'
    ...
    with Base() as base:
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(contractEndAdd,1)).is_true(),1060,
        ...
        base.diffAssert(lambda test: asserts(contractStatus).is_equal_to('OWNER_DEFAULT'), 1060,
        ...
        if sqlbase.waitData(breachAchievementSql,1):
            ...
            base.diffAssert(lambda test: asserts(breachAchievementInfo[0]).is_equal_to('N'), 1060,
                            u'%s:出租合同 %s 退租违约业绩状态异常，期望值 N 实际值 %s' % (fileName, contractNum, breachAchievementInfo[0]))
            base.diffAssert(lambda test: asserts(breachAchievementInfo[1]).is_equal_to(breach_money), 1060,
                            u'%s:出租合同 %s 退租违约业绩金额异常，期望值 %s 实际值 %s' % (fileName, contractNum, breach_money, breachAchievementInfo[1]))
        else:
        ...
        breahAchievementDetailSql = "select * from contract_achievement ca inner join contract_achievement_detail cad on ca.achieve_id=cad.achieve_id where ca.contract_category='OWNER_DEFAULT_BREACH' " \
        ...
        base.diffAssert(lambda test: asserts(achievementDetialConunt).is_not_equal_to(0), 1060,
                        u'%s:出租合同 %s 退租终止违约业绩分成明细异常' % (fileName, contractNum))
