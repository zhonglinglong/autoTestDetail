...
def test_1120():
    ...
    # describe：出租合同退租终止结算生成扣回业绩，记录同步到预估业绩排行榜
    # data：1、出租合同状态为有效；2、出租合同审核状态为已复审；
    # result：1、合同状态变为退租；2、生成一条扣回业绩，状态为未生效；3、扣回业绩分成记录插入到预估业绩排行榜；

    fileName = 'apartmentAchievement_1120'
    ...
    with Base() as base:
        ...
        contractEndAdd = "SELECT ace.end_contract_num FROM apartment_contract ac,apartment_contract_end ace WHERE ac.contract_id = ace.contract_id " \
                         "and ace.audit_status='NO_AUDIT' AND ace.end_type='OWNER_DEFAULT'and ace.deleted=0 and ac.contract_num='%s'" % contractNum
        base.diffAssert(lambda test: asserts(sqlbase.waitData(contractEndAdd,1)).is_true(),1120,
        ...
        base.diffAssert(lambda test: asserts(contractStatus).is_equal_to('OWNER_DEFAULT'), 1120,
                        u'%s:出租合同 %s 终止结算后状态异常异常，期望值 OWNER_DEFAULT 实际值 %s' % (fileName, contractNum, contractStatus))
        #扣回业绩检查
        backAchievementSql = "select is_active,achieve_id,accounting_time from back_achievement where contract_num='%s' and deleted=0 and contract_end_type='OWNER_DEFAULT' " % contractNum
        if sqlbase.waitData(backAchievementSql,1):
            backAchievementInfo = sqlbase.serach(backAchievementSql)
            base.diffAssert(lambda test: asserts(backAchievementInfo[0]).is_equal_to('N'), 1120,
                            u'%s:出租合同 %s 退租扣回业绩状态异常，期望值 N 实际值 %s' % (fileName, contractNum, backAchievementInfo[0]))
            # base.diffAssert(lambda test: asserts(breachAchievementInfo[1]).is_equal_to(breach_money), 1120,
            #                 u'%s:出租合同 %s 退租扣回业绩金额异常，期望值 %s 实际值 %s' % (fileName, contractNum, breach_money, breachAchievementInfo[1]))
        else:
            consoleLog(u'%s:出租合同 %s 退租终止生成扣回业绩异常，执行SQL：%s' % (fileName, contractNum, backAchievementSql))
        ...
        backAchievementDetailSql = "select * from contract_achievement ca inner join apartment_contract ac on ac.contract_id=ca.contract_id and ca.achieve_id='%s' where ca.deleted=0 " \
                                   "and ca.contract_category='OWNER_DEFAULT_BACK'" % backAchievementInfo[1]
        achievementDetialConunt = sqlbase.get_count(backAchievementDetailSql)
        base.diffAssert(lambda test: asserts(achievementDetialConunt).is_not_equal_to(0), 1120,
                        u'%s:出租合同 %s 退租终止扣回业绩分成明细异常' % (fileName, contractNum))
