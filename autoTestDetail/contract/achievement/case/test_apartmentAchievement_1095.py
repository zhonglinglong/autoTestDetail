...
def test_1095():
    ...
    # describe：违约业绩审核后修改应终止结算转租费+100，原业绩不变
    # data：1、出租终止类型为转租；2、出租终止结算未复审；3、业绩状态已审核；
    # result：1、业绩不变

    fileName = 'apartmentAchievement_1095'
    ...
    with Base() as base:
        ...
        if not sqlbase.waitData(achievementSql, 1):
        ...
        for i in range(3):
            ...
            time.sleep(1)
        # 业绩生效检查
        achievementsql = "select aca.is_active,aca.audit_status,aca.contract_audit_status from apartment_contract_achievement aca " \
                          "inner join apartment a on a.apartment_code=aca.house_code and a.apartment_id='%s' where aca.contract_num='%s' and aca.deleted=0 and aca.is_active='Y'and aca.accounting_time is not null" % (apartmentId, apartmentContractNum)
        base.diffAssert(lambda test: asserts(sqlbase.waitData(achievementsql,1)).is_true(), 1095,
        ...
        # 违约业绩检查
        breachAchievementSql = "select is_active,accounting_money,achieve_id from breach_achievement where breach_num='%s' and deleted=0 and breach_type='CUSTOMER_SUBLET' " \
        ...
        if sqlbase.waitData(breachAchievementSql, 1):
            ...
            base.diffAssert(lambda test: asserts(breachAchievementInfo[1]).is_equal_to(zhuanzu_money), 1095,
                            u'%s:出租合同 %s 转租违约业绩金额异常，期望值 %s 实际值 %s' % (
                            fileName, apartmentContractNum, zhuanzu_money, breachAchievementInfo[1]))
        else:
            consoleLog(u'%s:出租合同 %s 转租终止生成违约业绩异常，执行SQL：%s' % (fileName, apartmentContractNum, breachAchievementSql))
        # 业绩审核
        ...
        zhuanzu_money_new = str(float(zhuanzu_money)+100)
        ...
        # 获取修改后业绩数据
        time.sleep(10)
        breachAchievementNewSql = "select is_active,accounting_money,achieve_id from breach_achievement where breach_num='%s' and deleted=0 and breach_type='CUSTOMER_SUBLET'" % apartmentContractNum
        breachAchievementNewInfo = sqlbase.serach(breachAchievementNewSql)
        base.diffAssert(lambda test: asserts(breachAchievementNewInfo[1]).is_equal_to(zhuanzu_money), 1095,
                        u'%s:出租合同 %s 转租终止违约修改转租费后业绩金额异常，期望值 %s 实际值 %s' % (fileName, apartmentContractNum, zhuanzu_money, breachAchievementNewInfo[1]))
        ...
        breahAchievementDetailSql = "select ca.receivable from contract_achievement ca inner join contract_achievement_detail cad on ca.achieve_id=cad.achieve_id inner join breach_achievement ba " \
                                    "on ba.achieve_id=ca.achieve_id and ba.breach_type='CUSTOMER_SUBLET' and ba.deleted=0 and ba.breach_num='%s' where ca.contract_category='CUSTOMER_SUBLET' " \
                                    "and ca.deleted=0 " % apartmentContractNum
        prebreachAchievement = sqlbase.serach(breahAchievementDetailSql)[0]
        base.diffAssert(lambda test: asserts(prebreachAchievement).is_equal_to(zhuanzu_money), 1095,
                        u'%s:出租合同 %s 转租终止违约修改转租费后,业绩分成明细业绩金额异常，实际值 %s 期望值 %s' % (
                        fileName, apartmentContractNum, prebreachAchievement, breach_money))
