...
def test_1065():
    ...
    with Base() as base:
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(contractEndAdd, 1)).is_true(), 1065,
        ...
        base.diffAssert(lambda test: asserts(contractStatus).is_equal_to('RETREATING'), 1065,
                        u'%s:委托合同 %s 正退终止结算后状态异常，期望值 RETREATING 实际值 %s' % (fileName, contractNum, contractStatus))
        # 违约业绩检查
        ...
        base.diffAssert(lambda test: asserts(sqlbase.get_count(breachAchievementSql)).is_equal_to(0), 1065,
