...
def test_1055():
    ...
    # describe：在委托合同详情中，修改业绩核算周期对应时间段的委托合同租金策略中的租金，已审核的业绩数据不变
    # data：1、业绩审核状态为待审核；1、业绩状态为已审核；3、提前记录原业绩中的核算收进价和差价业绩；
    # result：1、业绩中所有字段都不发生变化；

    fileName = 'apartmentAchievement_1055'
    ...
    with Base() as base:
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(achievementSql, 1)).is_true(), 1055,
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(achievementsqlb, 1)).is_true(), 1055,
        ...
        base.input_text(houseContractPage.addHouseContractMould['contract_strategy1_loc'], 4000)  # 第二年租金策略
        ...
        base.diffAssert(lambda test: asserts(rent_cost).is_equal_to(accountingCostOld), 1055,
                        u'%s:出租合同 %s 业绩对应委托成本修改后已审核业绩中核算收进价异常，修改前 %s 修改后 %s' % (fileName, apartmentContractNum, accountingCostOld, rent_cost))
        base.diffAssert(lambda test: asserts(profits_fee).is_equal_to(profits_feeOld), 1055,
