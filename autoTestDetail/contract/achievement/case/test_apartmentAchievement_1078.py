...
def test_1078():
    ...
    # describe：出租合同退租终止结算生成违约业绩，修改转租费+100，同步到业绩中
    # data：1、出租终止类型为退租；2、出租终止结算未复审；3、业绩状态未审核；
    # result：1、违约业绩比之前多100；2、分成人业绩同步变化；3、预估业绩排行榜同步更新；

    fileName = 'apartmentAchievement_1078'
    ...
    with Base() as base:
        ...
        if sqlbase.waitData(breachAchievementSql, 1):
            ...
            base.diffAssert(lambda test: asserts(breachAchievementInfo[1]).is_equal_to(breach_money), 1078,
        ...
        breach_money_new = float(breach_money)+100
        base.input_text(apartmentContractEndPage.addContractEndMould['remark_loc'], 'AutoTest2')  # 备注
        base.dblclick(apartmentContractEndPage.addContractEndMould['weiyuejin_loc'], index=12)  # 违约金
        base.input_text(apartmentContractEndPage.addContractEndMould['receivable_money_loc'], str(breach_money_new))  # 应收违约金
        base.click(apartmentContractEndPage.addContractEndMould['save_button'])  # 保存
        base.check_submit()
        # 更新业绩信息
        time.sleep(10)
        breachAchievementNewSql = "select is_active,accounting_money,achieve_id from breach_achievement where breach_num='%s' and deleted=0 and breach_type='OWNER_DEFAULT_BREACH'" % contractNum
        breachAchievementNewInfo = sqlbase.serach(breachAchievementNewSql)
        base.diffAssert(lambda test: asserts(breachAchievementNewInfo[1]).is_equal_to(str(breach_money_new)), 1078,
                        u'%s:出租合同 %s 转租终止违约修改转租费后，违约业绩异常，期望值 %s 实际值 %s' % (fileName, contractNum, breach_money_new, breachAchievementNewInfo[1]))
        # base.diffAssert(lambda test: asserts(personAchievementNew).is_not_equal_to(personAchievementOld), 1078,
        ...
        breahAchievementDetailSql = "select ca.receivable from contract_achievement ca inner join contract_achievement_detail cad on ca.achieve_id=cad.achieve_id inner join breach_achievement ba " \
                                    ...
                                    "where ca.contract_category='OWNER_DEFAULT_BREACH' and ca.deleted=0 " % contractNum
        prebreachAchievement = sqlbase.serach(breahAchievementDetailSql)[0]
        base.diffAssert(lambda test: asserts(prebreachAchievement).is_equal_to(str(breach_money_new)), 1078,
                        u'%s:出租合同 %s 转租终止违约修改转租费后,业绩分成明细业绩金额异常，实际值 %s 期望值 %s' % (fileName, contractNum, prebreachAchievement, breach_money_new))
