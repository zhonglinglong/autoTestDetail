...
def test_1083():
    ...
    fileName = 'apartmentAchievement_1083'
    achievementSql = "select ba.breach_num,ba.apartment_code,ba.breach_type,ba.achieve_id from breach_achievement ba inner join apartment a on a.apartment_code=ba.apartment_code " \
                     "and city_code=330100 where ba.is_active='Y' and ba.audit_status='AUDIT' AND ba.deleted=0 order by rand() limit 1"
    ...
    breachNum = info[0]
    consoleLog(u'%s:使用合同 %s 违约业绩做审核' % (fileName,breachNum))
    ...
    with Base() as base:
        ...
        base.check_submit()
        achievementStatus = "select * from breach_achievement where is_active='Y' and audit_status='APPROVED' AND deleted=0 " \
                            "and achieve_id='%s'" % info[3]
        if not sqlbase.waitData(achievementStatus, 1):
            consoleLog(u'%s:出租合同 %s 业绩审核异常'% (fileName, breachNum),'e')
        base.dblclick(apartmentAchievementPage.searchContractMould['tr_contract'],
                      checkLoc=apartmentAchievementPage.detailAchievementMoudle['breach_num_loc'])  # 点击第一条数据等待详情页面完全显示
        try:
        ...
        for i in range(achievementDetailCount):
            ...
            audit_status = u'已审核'
            base.diffAssert(lambda test: asserts(achievement_audit_status).is_equal_to(audit_status), 1083,
