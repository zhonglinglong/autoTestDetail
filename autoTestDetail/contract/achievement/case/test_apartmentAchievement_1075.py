...
def test_1075():
    ...
    # describe：已审核的违约业绩出租合同转租终止结算改成退租，修改转租费输入6666，原业绩不变
    # data：1、出租终止类型为转租；2、业绩生效且已审核
    # result：1、业绩不变

    fileName = 'apartmentAchievement_1075'
    ...
    with Base() as base:
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(achievementsql,1)).is_true(), 1075,
        ...
        # 违约业绩检查
        breachAchievementSql = "select is_active,accounting_money,achieve_id from breach_achievement where breach_num='%s' and deleted=0 and breach_type='OWNER_DEFAULT_BREACH' and is_active='Y'" % apartmentContractNum
        if sqlbase.waitData(breachAchievementSql, 1):
            breachAchievementInfo = sqlbase.serach(breachAchievementSql)
            base.diffAssert(lambda test: asserts(breachAchievementInfo[1]).is_equal_to(breach_money), 1075,
                            u'%s:出租合同 %s 转租违约业绩金额异常，期望值 %s 实际值 %s' % (fileName, apartmentContractNum, breach_money, breachAchievementInfo[1]))
        else:
        ...
        # 获取修改后业绩数据
        time.sleep(10)
        breachAchievementSql = "select is_active,accounting_money,achieve_id from breach_achievement where breach_num='%s' and deleted=0 and is_active='Y'" \
                               "and breach_type='OWNER_DEFAULT_BREACH'" % apartmentContractNum
        breachAchievementInfo = sqlbase.serach(breachAchievementSql)
        base.diffAssert(lambda test: asserts(breachAchievementInfo[1]).is_equal_to(breach_money), 1075,
                        u'%s:出租合同 %s 违约业绩金额异常，期望值 %s 实际值 %s' % (fileName, apartmentContractNum, breach_money, breachAchievementInfo[1]))
        ...
        breahAchievementDetailSql = "select ca.receivable from contract_achievement ca inner join contract_achievement_detail cad on ca.achieve_id=cad.achieve_id inner " \
                                    "join breach_achievement ba on ba.achieve_id=ca.achieve_id and ba.breach_type='OWNER_DEFAULT_BREACH' and ba.deleted=0 and ba.breach_num='%s' " \
        ...
        base.diffAssert(lambda test: asserts(prebreachAchievement).is_equal_to(breach_money), 1075,
                        u'%s:出租合同 %s 退租终止违约修改成转租并修改转租费后,业绩分成明细业绩金额异常，实际值 %s 期望值 %s' % (
