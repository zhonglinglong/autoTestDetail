...
def test_1046():
    ...
    with Base() as base:
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(achievementSql, 1)).is_true(), 1046,
        ...
        base.diffAssert(lambda test:asserts(rent_cost_new).is_not_equal_to(rent_cost_old), 1046,
                        u'%s:出租合同 %s 对应委托免租期修改后业绩中核算收进价异常，修改前 %s 修改后 %s' % (fileName, apartmentContractNum, rent_cost_old, rent_cost_new))
        base.diffAssert(lambda test: asserts(profits_fee_new).is_not_equal_to(profits_fee_old), 1046,
