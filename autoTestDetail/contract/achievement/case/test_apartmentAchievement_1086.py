...
def test_1086():
    ...
    # describe：已生效且已审核的违约业绩删除终止结算后业绩失效
    # data：1、业绩已审核；2、终止结算未复审；
    # result：1、业绩变成已失效；2、状态同步到预估业绩排行榜；

    fileName = 'apartmentAchievement_1086'
    ...
    with Base() as base:
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(achievementsql, 1)).is_true(), 1086,
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(breachAchievementSql,1)).is_true(), 1086,
                        u'%s:出租合同 %s 违约业绩生效异常，执行SQL：%s' % (fileName, apartmentContractNum, breachAchievementSql))
        # 业绩审核
        ...
        base.check_submit()
        # 终止结算删除后
        breachAchievementDelSql = "select is_active,accounting_money,achieve_id from breach_achievement where breach_num='%s' and deleted=0 and breach_type='OWNER_DEFAULT_BREACH' " \
                                  "and is_active='INVALID'" % apartmentContractNum
        base.diffAssert(lambda test: asserts(sqlbase.waitData(breachAchievementDelSql, 1)).is_true(), 1086,
                        u'%s:出租合同 %s 租违约业绩审核删除终止结算后业绩异常，执行SQL：%s' % (fileName, apartmentContractNum, breachAchievementDelSql))
        ...
        base.diffAssert(lambda test: asserts(breachAchievementStatus).is_equal_to('INVALID'), 1086,
                        u'%s:出租合同 %s 退租违约业绩审核删除终止结算后,业绩分成明细业绩状态异常，期望值 INVALID 实际值 %s' % (fileName, apartmentContractNum, breachAchievementStatus))
