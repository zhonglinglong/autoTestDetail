...
def test_1067():
    ...
    with Base() as base:
        ...
        # 违约业绩检查
        breachAchievementSql = "select is_active,accounting_money,achieve_id,accounting_time from breach_achievement where breach_num='%s' and deleted=0 and is_active='Y' " \
        ...
        if sqlbase.waitData(breachAchievementSql, 1):
            ...
            base.diffAssert(lambda test: asserts(breachAchievementInfo[3]).is_not_empty(), 1067,
                            u'%s:出租合同 %s 退租违约业生效核发月份生成异常' % (fileName, apartmentContractNum))  #  核发业绩排行榜取值处
        else:
            consoleLog(u'%s:出租合同 %s 退租终止生成违约业绩异常，执行SQL：%s' % (fileName, apartmentContractNum, breachAchievementSql))
        ...
        breahAchievementDetailSql = "select * from contract_achievement ca inner join contract_achievement_detail cad on ca.achieve_id=cad.achieve_id where ca.contract_category='OWNER_DEFAULT_BREACH' " \
                                    "and ca.deleted=0 and ca.achieve_id='%s'and ca.achievement_month='%s'" % (breachAchievementInfo[2], breachAchievementInfo[3])
        achievementDetialConunt = sqlbase.get_count(breahAchievementDetailSql)
        base.diffAssert(lambda test: asserts(achievementDetialConunt).is_not_equal_to(0), 1067,
                        u'%s:出租合同 %s 退租终止违约业绩分成明细异常' % (fileName, apartmentContractNum))
        ...
        base.type_select(apartmentAchievementPage.searchContractMould['category_loc'], 'OWNER_DEFAULT_BREACH')  # 分类：客户退租
        ...
        for i in range(achievementDetialConunt):
            ...
            base.diffAssert(lambda test: asserts(achievement_month).is_equal_to(breachAchievementInfo[3]), 1067,
