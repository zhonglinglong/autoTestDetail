...
def test_1103():
    ...
    # describe：委托合同公司违约终止结算
    # data：1、委托合同已复审；2、合同状态为有效；3、委托合同下无有效的出租合同；
    # result：1、合同状态变为公司违约；2、终止结算提交成功

    fileName = 'apartmentContract_1103'
    ...
    with Base() as base:
        ...
        base.diffAssert(lambda test: asserts(sqlbase.waitData(contractEndAdd, 1)).is_true(), 1103,
                        u'%s:委托合同 %s 公司违约终止结算新增异常，执行SQL：%s' % (fileName, contractNum, contractEndAdd))
        # 合同状态检查
        ...
        base.diffAssert(lambda test: asserts(contractStatus).is_equal_to('CORPORATE_DEFAULT'), 1103,
                        u'%s:委托合同 %s 公司违约终止结算后状态异常，期望值 OWNER_DEFAULT 实际值 %s' % (fileName, contractNum, contractStatus))
