...
def test_1107():
    ...
    # describe：已续委托合同不能做终止结算
    # data：1、委托合同已续
    # result：1、终止结算失败

    fileName = 'apartmentContract_1107'
    contractSql = "select contract_num from house_contract where is_active='Y' and contract_status='CONTINUED' and deleted=0 and city_code=330100 order by rand() limit 1"
    ...
    contractNum = info[0]
    consoleLog(u'%s:取随机已续委托合同 %s 做终止结算' % (fileName, contractNum))
    ...
    with Base() as base:
        ...
        message = base.script(
            ...
            True)  # 获取提示信息
        messagehope = u'委托合同已续不能终止'
        base.diffAssert(lambda test: asserts(message).is_equal_to(messagehope),1107,
