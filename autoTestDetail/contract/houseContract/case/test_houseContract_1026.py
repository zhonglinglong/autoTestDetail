...
def test_1026():
    ...
    with Base() as base:
        ...
        newContractNum = 'AutoTestHX' + '-' + time.strftime('%m%d%H%M%S');consoleLog(u'续签合同号为 %s' % newContractNum)   # 定义合同编号
        ...
        base.type_date(houseContractPage.typeMould['entrust_start_date'], contractInfo[9])  # 委托起算日:原合同到期后一月
        ...
        base.type_date(houseContractPage.typeMould['first_pay_date'], contractInfo[9])  # 首次付款日:原合同到期后一月
        base.type_date(houseContractPage.typeMould['second_pay_date'], contractInfo[11])  # 第二次付款日:原合同到期后二个月
        ...
        base.diffAssert(lambda test: asserts(sqlbase.serach(contractSql)[0]).is_equal_to('CONTINUED'),1026,
