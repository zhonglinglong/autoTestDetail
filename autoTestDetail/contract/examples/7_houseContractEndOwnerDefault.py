...
def houseContractmentEnd():
    ...
    # describe： 委托合同业主违约终止结算，生成违约业绩
    # data：委托合同下无承租周期在当前日期之后的出租合同，委托合同到期日在当前日期之后
    # result：终止结算通过，生成违约业绩
    ...
    if sqlbase.get_count(sql) > 0:
        ...
        consoleLog(u'委托合同 %s 做新增终止结算操作' % contractNum)
    else:
    ...
    with Base() as base:
        ...
        base.input_text((houseContractEndPage.addContractEndMould['end_num_loc']), endnum)#终止协议号
        base.type_date(houseContractEndPage.typeMould['end_date'], today)  # 终止日期
        # 结算扣款
        ...
        base.type_date(houseContractEndPage.typeMould['receivable_date'], today)  # 应收日期
        # 打款信息
        ...
        contractEndAdd="SELECT * FROM house_contract ,house_contract_end WHERE house_contract.contract_id = house_contract_end.contract_id AND house_contract_end.end_type='OWNER_DEFAULT' " \
        ...
        base.check_submit()
        contractEndAud = "SELECT * FROM house_contract ,house_contract_end WHERE house_contract.contract_id = house_contract_end.contract_id AND house_contract_end.end_type='OWNER_DEFAULT' " \
        ...
        if is_cative == u'生效'.encode('utf-8'):
            consoleLog(u'终止结算复审通过，违约业绩生效。')
        else:
            consoleLog(u'违约业绩非生效状态，业务异常！','e')
