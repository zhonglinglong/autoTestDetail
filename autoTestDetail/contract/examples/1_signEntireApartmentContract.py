...
@log
def signEntireApartmentContract():
    """客户整租合同签约审核主流程"""

    # describe：客户整租合同签约审核主流程
    # data：有效的租前客户，有效整租房源
    # result：新增整租合同成功，审核通过
    ...
    with Base() as base:
        ...
        consoleLog(u'使用租客 %s 签约承租合同' % customerCode, level='i')
        base.staleness_of(apartmentContractPage.customerSignMould['tr_customer'])  # 搜索等待列表刷新
        ...
        randomApartment = "SELECT a.apartment_code,a.apartment_id,hc.contract_num,hc.contract_id FROM apartment a INNER JOIN house_contract hc " \
                          ...
                          "WHERE a.deleted = 0 AND a.rent_price > 0 AND a.city_code = 330100 AND hc.entrust_type = 'ENTIRE' AND NOT EXISTS ( SELECT 1 FROM apartment_contract_relation " \
        ...
        consoleLog(u'使用房源 %s 签约承租合同' % apartmentCode, level='i')
        ...
        randomHouseContract = sqlbase.serach(
            "SELECT entrust_start_date,entrust_end_date,date(sysdate()),date_add(date(sysdate()), interval 1 DAY) from house_contract where contract_num = '%s'" %
            info[2].encode('utf-8'))
        ...
        contractAdd = "SELECT * FROM apartment_contract , house ,apartment WHERE apartment_contract.house_id = house.house_id " \
                      ...
                      "and apartment_contract.contract_type = 'NEWSIGN' AND apartment_contract.entrust_type='ENTIRE' AND apartment_contract.is_active='Y' " % (
        ...
        base.click(apartmentContractPage.searchContractMould['entire_loc'])  # 点击整租
        base.wait_element(apartmentContractPage.searchContractMould['entire_tr_contract']) #等待列表出现
        base.input_text(apartmentContractPage.searchContractMould['entire_contract_num_loc'], contractNum)  # 输入合同编号
        ...
        base.dblclick(apartmentContractPage.searchContractMould['entire_tr_contract'], checkLoc=apartmentContractPage.addApartmentContractMould['contract_num_loc'])  # 双击第一条数据
        ...
        time.sleep(3)
        base.dblclick(apartmentContractPage.searchContractMould['entire_tr_contract'])  # 双击第一条数据
        for i in range(5):
            try:
                base.click(apartmentContractPage.addApartmentContractMould['tab_info_loc'], index=4)  # 租客详情
                break
        ...
        base.click(apartmentContractPage.addApartmentContractMould['contract_audit_confirm'])  # 确定
        base.check_submit()  # 等待提交完成
        contractAudit = "SELECT * FROM apartment_contract , house ,apartment WHERE apartment_contract.house_id = house.house_id " \
                        ...
                        "and apartment_contract.contract_type = 'NEWSIGN' AND apartment_contract.entrust_type='ENTIRE' AND apartment_contract.is_active='Y' " % (apartmentCode, contractNum)
        if sqlbase.get_count(contractAudit) == 1:#数据检查
            ...
            return
        # 生成的业绩变成已生效
        achievementsqlb = "select * from apartment_contract_achievement where contract_num='%s' and is_active='Y' and accounting_num=1 and audit_status='AUDIT' and deleted=0 " % contractNum.encode('utf-8')
        for i in range(5):
